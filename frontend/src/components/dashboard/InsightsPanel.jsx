...
const LightbulbIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.03-7.03l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707.707M12 21v-1m0-16a4 4 0 00-4 4h8a4 4 0 00-4-4z" />
    </svg>
);
...
function InsightsPanel() {
  ...
  console.log('[InsightsPanel] Data from useApi hook:', { hypothesisData, loading, error });
      ...
      {hypothesisData ? (
        <div className="animate-fade-in">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <h5 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Source Data Finding</h5>
            <div className="mt-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
              <p>
                A correlation of <strong>{hypothesisData.source_finding.correlation.toFixed(2)}</strong> was found between 
                <span className="font-semibold text-indigo-600"> {hypothesisData.source_finding.variable}</span> and the population of 
                <span className="font-semibold text-indigo-600"> {hypothesisData.source_finding.species_name}</span>.
              </p>
            </div>
          </div>
        </div>
